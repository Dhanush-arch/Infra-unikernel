...
menuentry "unikernel" {
    multiboot2 /boot/unikernel.elf dhcp=off ip4={{ ip4 }} gateway_ip4={{ gateway_ip4 }} pci_device=0:4
    savedefault 0
    boot
}
...
def main():
    ...
    if Path("/unikernel.dump").exists():
        with Path("/unikernel.dump").open("rb") as f:
            ...
            if magic == 0xDEADBEEE:
                ...
                with smtplib.SMTP("localhost") as smtp:
                    m = "Subject: unikernel has crashed\n\n"
                    m += crash_log
                    smtp.sendmail(
                        "unikernel-crash-report@dhanusharch.me",
                        "email@dhanusharch.me",
                        m,
                    )

    subprocess.run(["dd", "if=/dev/zero", "of=/unikernel.dump", "bs=1024", "count=64"])
    ...
    image_url = f"http://{bastion_ip}/uploads/unikernel.elf"
    print(f"image URL: {image_url}")
    resp = requests.get(image_url, allow_redirects=True)
    if resp.status_code != 200:
        sys.exit(f"failed to download an image from {image_url}")
    with open("/boot/unikernel.elf", "wb") as f:
        f.write(resp.content)

    ip4_addr = requests.get(
        "http://169.254.169.254/metadata/v1/interfaces/private/0/ipv4/address"
    ).text.strip()
    gateway_ip4 = requests.get(
        "http://169.254.169.254/metadata/v1/interfaces/private/0/ipv4/gateway"
    ).text.strip()
    netmask = requests.get(
        "http://169.254.169.254/metadata/v1/interfaces/private/0/ipv4/netmask"
    ).text.strip()
    netmask_len = ipaddress.IPv4Network(f"0.0.0.0/{netmask}").prefixlen

    with open("/etc/grub.d/40_custom", "w") as f:
        f.write(
            GRUB_40_CUSTOM.render(
                ip4=f"{ip4_addr}/{netmask_len}", gateway_ip4=gateway_ip4
            )
        )

    # Boot unikernel in the next boot. which means we need to reboot
    subprocess.run(["update-grub"])
    subprocess.run(["grub-reboot", "unikernel"])
